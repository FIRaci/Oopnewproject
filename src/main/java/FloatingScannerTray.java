...
public class FloatingScannerTray extends JWindow {
    ...
    // Example main method for testing the FloatingScannerTray independently
    public static void main(String[] args) {
        // Apply a Look and Feel for better appearance during testing
        try {
        ...
        SwingUtilities.invokeLater(() -> {
            // Create a dummy owner frame for testing context
            JFrame testOwner = new JFrame("Test Owner Frame");
            testOwner.setSize(800, 600);
            testOwner.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            testOwner.setLocationRelativeTo(null);
            testOwner.setVisible(true);

            // Get and show the scanner tray instance
            FloatingScannerTray tray = FloatingScannerTray.getInstance();
            // tray.setOwner(testOwner); // Owner is now set in getInstance if possible
            tray.setVisible(true);

            // Simulate showing it again to test positioning
            Timer timer = new Timer(3000, e -> {
                System.out.println("Hiding and showing tray...");
